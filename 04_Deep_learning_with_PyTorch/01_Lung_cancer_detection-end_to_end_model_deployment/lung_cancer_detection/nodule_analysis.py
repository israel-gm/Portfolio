...
from lung_cancer_detection.data.dsets import LunaDataset, getCt, getCandidateInfoDict, getCandidateInfoList, CandidateInfoTuple
from p2ch13.model import UNetWrapper

import lung_cancer_detection.model
...
logging.getLogger("lung_cancer_detection.data.dsets").setLevel(logging.WARNING)
...
class NoduleAnalysisApp:
    ...
    def initModels(self):
        ...
        model_cls = getattr(lung_cancer_detection.model, self.cli_args.cls_model)
        cls_model = model_cls()
        ...
        if self.cli_args.malignancy_path:
            model_cls = getattr(lung_cancer_detection.model, self.cli_args.malignancy_model)
