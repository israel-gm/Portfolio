...
from app.utils.disk import getCache
from app.utils.util import XyzTuple, xyz2irc
...
# .../workspace/media/luna/
LUNA_ROOT = WORKSPACE_ROOT / "media" / "luna"



raw_cache = getCache('classification_cache')
...
def getCandidateInfoList(requireOnDisk_bool=True):
    ...
    # the subsets yet.
    mhd_list = glob.glob(str(LUNA_ROOT / "subset*" / "*.mhd"))
    presentOnDisk_set = {os.path.split(p)[-1][:-4] for p in mhd_list}

    candidateInfo_list = []
    with open(LUNA_ROOT / "annotations_with_malignancy.csv", "r") as f:
    ...
    with open(LUNA_ROOT / "candidates.csv", "r") as f:
        for row in list(csv.reader(f))[1:]:
            series_uid = row[0]
...
class Ct:
    def __init__(self, series_uid):
        mhd_path = glob.glob(str(LUNA_ROOT / "subset*" / f"{series_uid}.mhd"))[0]
