...
from app.utils.util import enumerateWithEstimate
from app.datasets.segmentation_dsets import Luna2dSegmentationDataset
from app.datasets.classification_dsets import LunaDataset, getCt, getCandidateInfoDict, getCandidateInfoList, CandidateInfoTuple
from app.models.segmentation_model import UNetWrapper

import app.models.classification_model

from app.utils.logconf import logging
from app.utils.util import xyz2irc, irc2xyz

np.int = int  # 👈 jigm update: restore the deprecated alias for old code

log = logging.getLogger(__name__)
# log.setLevel(logging.WARN)
# log.setLevel(logging.INFO)
log.setLevel(logging.DEBUG)
logging.getLogger("app.datasets.segmentation_dsets").setLevel(logging.WARNING)
logging.getLogger("app.datasets.classification_dsets").setLevel(logging.WARNING)

# Path to this file → .../workspace/app/datasets/this_file.py
THIS_FILE = Path(__file__).resolve()
# .../workspace/
WORKSPACE_ROOT = THIS_FILE.parents[2]
# .../workspace/app/training/saved_weights/
WEIGHTS_ROOT = WORKSPACE_ROOT / "app" / "training" / "saved_weights"
...
class NoduleAnalysisApp:
    def __init__(self, sys_argv=None):
        ...
        parser.add_argument('--segmentation-path',
            ...
            nargs='?',
            default=WEIGHTS_ROOT /'seg_2020-01-26_19.45.12_w4d3c1-bal_1_nodupe-label_pos-d1_fn8-adam.best.state',
        )
        ...
        parser.add_argument('--classification-path',
            ...
            nargs='?',
            default=WEIGHTS_ROOT / 'cls_2020-02-06_14.16.55_final-nodule-nonnodule.best.state',
        )
    ...
    def initModelPath(self, type_str):
        local_path = str(WEIGHTS_ROOT / f"{type_str}_*_*_best.state")
        file_list = glob.glob(local_path)
        
        if not file_list:
            pretrained_path = str(WEIGHTS_ROOT / f"{type_str}_*_*_*.state")
    ...
    def initModels(self):
        ...
        model_cls = getattr(app.models.classification_model, self.cli_args.cls_model)
        cls_model = model_cls()
        ...
        if self.cli_args.malignancy_path:
            model_cls = getattr(app.models.classification_model, self.cli_args.malignancy_model)
